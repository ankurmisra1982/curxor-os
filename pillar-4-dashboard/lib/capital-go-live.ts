...
import { buildCapitalGrowthProfile } from "./capital-growth";
import {
  capitalGoLiveStepDetail,
  capitalGoLiveStepLabel,
} from "./capital-level-copy";
...
export async function buildCapitalGoLiveReport(): Promise<CapitalGoLiveReport> {
  const [fre, file, bridgeConfigured, settings] = await Promise.all([
  ...
  const step = (id: string, label: string, status: CapitalGoLiveStepStatus, detail: string): CapitalGoLiveStep => ({
    id,
    label: capitalGoLiveStepLabel(growth, id, label),
    status,
    detail: capitalGoLiveStepDetail(growth, id, detail),
  });
  ...
  steps.push(
    step(
      ...
      fre.initialized ? `${tradingMode} · ${fre.config.riskProfile ?? "balanced"}` : "Complete Capital Claw setup",
    ),
  );
  ...
  steps.push(
    step(
      "alpaca",
      "Alpaca paper bridge",
      bridgeConfigured ? "complete" : "warning",
      bridgeConfigured
  ...
  steps.push(
    step(
      ...
      tradingMode === "paper" || tradingMode === "dry_run" ? "complete" : "warning",
  ...
  steps.push(
    step(
      "rule",
      "First rule created",
      hasRule ? "complete" : "pending",
      hasRule ? `${file.rules.length} rule(s)` : "Create a WHEN/THEN rule or run Setup Wizard",
    ),
  );
  ...
  steps.push(
    step(
      "armed",
      "Rule armed",
      armed.length > 0 ? "complete" : hasRule ? "warning" : "pending",
      armed.length > 0 ? `${armed.length} armed` : "Arm a rule before Execute Trade",
    ),
  );
  ...
  steps.push(
    step(
      "first_fill",
      "First execution",
      hasAnyFill ? "complete" : armed.length > 0 ? "warning" : "pending",
      hasAnyFill
