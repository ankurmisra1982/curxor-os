...
export function PatronAskFab() {
  ...
  return createPortal(
    <button
      ...
      className="fixed bottom-6 right-6 z-[45] flex h-14 w-14 items-center justify-center rounded-full border border-cursor-glow bg-panel shadow-cursor transition hover:bg-void"
    >
      <span
      ...
      >
        P
        {unread ? (
          <span className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full bg-cursor-glow" aria-hidden />
        ) : null}
      </span>
