...
import { buildCapitalGoLiveReport } from "@/lib/capital-go-live";
...
const AGENT_TOOLS = [
  ...
  { id: "query_portfolio", params: ["query"], description: "Natural-language portfolio Q&A" },
  { id: "get_go_live_report", params: [], description: "Go Live checklist — demoReady, paperReady, steps" },
  { id: "walk_forward_backtest", params: ["ruleId"], description: "Walk-forward backtest for a rule" },
] as const;
...
export async function GET(request: Request): Promise<Response> {
  ...
  if (tool === "query_portfolio") {
    ...
    return Response.json({ ok: true, ...answerPortfolioQuery(query, status) });
  }

  if (tool === "get_go_live_report") {
    const goLive = await buildCapitalGoLiveReport();
    return Response.json({ ok: true, goLive });
  }

  if (tool === "walk_forward_backtest") {
    ...
    const walkForward = await walkForwardBacktest(rule);
    return Response.json({ ok: true, walkForward });
  }
